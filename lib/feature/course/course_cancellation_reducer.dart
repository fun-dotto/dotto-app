...
final class CourseCancellationReducer extends _$CourseCancellationReducer {
  ...
  Future<void> refresh() async {
    ...
    state = await AsyncValue.guard(
      () => _createState(isFiltered: current?.isFiltered ?? false),
    );
  }
  ...
  Future<void> toggleFilter() async {
    ...
    state = const AsyncLoading();
    state = await AsyncValue.guard(
      () => _createState(isFiltered: nextIsFiltered),
    );
  }

  Future<CourseCancellationState> _createState({
    ...
    if (isFiltered) {
      ...
      subjectIds = BuiltList<String>(
        courseRegistrations.map((r) => r.subject.id),
      );
    }
